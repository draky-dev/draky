...
get_recipe_path() {
  echo -n "$(get_env_path "$1/docker-compose.recipe.yml")"
}

get_compose_path() {
  echo -n "$(get_env_path "$1/docker-compose.yml")"
}
...
DEFAULT_ENV=dev
DEFAULT_ENV_PATH="$(get_env_path ${DEFAULT_ENV})"
DEFAULT_ENV_RECIPE_PATH="$(get_recipe_path ${DEFAULT_ENV})"
...
EOF
  run ${DRAKY} env build
  [[ "$status" == 0 ]]
  [ -f "${DEFAULT_ENV_COMPOSE_PATH}" ]
  grep -q "image: php-image" "$DEFAULT_ENV_COMPOSE_PATH"
}
...
EOF
  "${DRAKY}" env build
  run cat $DEFAULT_ENV_PATH/docker-compose.yml
  [[ "$output" == *"# This file is autogenerated"* ]]
}
...
EOF
  ${DRAKY} env build
  grep -q "../../services/php/./test-volume" "$DEFAULT_ENV_COMPOSE_PATH"
  # Test if dict volume definitions are also handled correctly.
  grep -q "source: ../../services/php/./test-2-volume" "$DEFAULT_ENV_COMPOSE_PATH"
}
...
EOF
  ${DRAKY} env build
  grep -q "../../services/php/." "$DEFAULT_ENV_COMPOSE_PATH"
}
...
EOF
  ${DRAKY} env build
  grep -q -- "- $NAMED_VOLUME_1:" "$DEFAULT_ENV_COMPOSE_PATH"
}
...
EOF
  ...
  ${DRAKY} env build
  grep -q -- "version: '3.4'" "$DEFAULT_ENV_COMPOSE_PATH"
}
...
EOF
  ${DRAKY} env build
  run ! grep -q "draky:" "$DEFAULT_ENV_COMPOSE_PATH"
}
...
EOF
  ...
  cat > "$(dirname "$DEFAULT_ENV_COMPOSE_PATH")/Dockerfile" << EOF
FROM alpine
...
EOF
  ${DRAKY} env build
  grep -q "../../services/php/./Dockerfile" "$DEFAULT_ENV_COMPOSE_PATH"
  # Test if dict volume definitions are also handled correctly.
  grep -q "../../services/php/./context" "$DEFAULT_ENV_COMPOSE_PATH"
}
...
EOF
  run "${DRAKY}" env name
  [[ "$output" == "${TEST_ENV_1}"$'\r' ]]
  mkdir -p "$(get_env_path $TEST_ENV_2)"
  switch_environment "${TEST_ENV_2}"
  run "${DRAKY}" env name
  [[ "$output" == "${TEST_ENV_2}"$'\r' ]]
}
...
@test "DRAKY_ENV: Environment-specific custom commands" {
  _initialize_test_project
